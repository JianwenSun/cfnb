...
1. 复制你的 GitHub Raw 链接：
   ...
   https://raw.githubusercontent.com/你的用户名/仓库名/refs/heads/分支名/ip.txt
   ```
2. 打开 EdgeTunnel 控制面板，点击左侧菜单栏的 **“优选订阅生成”**。
3. 在右侧 **“自定义订阅模式”** 区域，选择 **“自定义订阅（支持汇聚订阅）”**。
4. 在下方的输入框中，粘贴上一步获取的 GitHub Raw 链接。
   > 💡 如需指定端口，可在链接后添加 `?port=443`。
5. 在 **“默认端口”** 输入框中填写端口号（通常为 `443`）。
6. （可选）勾选 **“将优选作为 PROXYIP”**。
7. 点击 **“订阅接口”** 按钮，系统将验证 API 并拉取节点列表。
8. 检查无误后，点击 **“保存”** 按钮完成配置。

### 方法二：手动替换 EdgeTunnel 节点配置

1. 打开项目生成的 `ip.txt` 文件，复制其中任意节点（格式为 `IP:端口#国家`）。
2. 打开 EdgeTunnel 控制面板，进入 **“优选订阅生成”** → **“自定义订阅模式”**。
3. 在输入框中直接粘贴节点地址，每行一个，例如：
   ```
   104.16.x.x:443#US
   162.159.x.x:443#HK
   ```
...
如果您已启用 Cloudflare DNS 批量更新，则可以直接将您的子域名（如 `你的域名.dpdns.org`）填入 EdgeTunnel 的 **“Cloudflare CDN 访问设置”** 中：

1. 打开 EdgeTunnel 控制面板，进入 **“Cloudflare CDN 访问设置”** 页面。
2. 在 **“PROXYIP”** 输入框中，填入您的子域名（例如 `你的域名.dpdns.org`）。
3. 点击 **“保存”** 按钮。
4. 该域名会自动解析到当前最优的多个 IP 之一，实现零配置动态切换。
