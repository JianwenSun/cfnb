...
程序运行完成后，`ip.txt` 会被推送到 GitHub，生成如下 Raw 链接：

```
https://raw.githubusercontent.com/你的用户名/仓库名/refs/heads/分支名/ip.txt
```

### 步骤二：在 EdgeTunnel 中配置优选订阅

1. 打开 EdgeTunnel 控制面板，点击左侧菜单栏的 **“优选订阅生成”**。
2. 在右侧 **“自定义订阅模式”** 区域，选择 **“自定义订阅（支持汇聚订阅）”**。
3. 在下方的输入框中，粘贴上一步获取的 GitHub Raw 链接，例如：
   ```
   https://raw.githubusercontent.com/你的用户名/仓库名/refs/heads/分支名/ip.txt
   ```
   > 💡 如需指定端口，可在链接后添加 `?port=443`，如不指定则使用下方“默认端口”设置。
4. 在 **“默认端口”** 输入框中填写端口号（通常为 `443`）。
5. （可选）勾选 **“将优选作为 PROXYIP”**，将筛选出的节点用作代理 IP。
6. 点击 **“订阅接口”** 按钮，系统将自动验证 API 地址并拉取节点列表。
   - 验证成功后，节点列表会显示在 **“接口结果”** 区域。
7. 检查无误后，点击 **“保存”** 按钮完成配置。

### 步骤三：使用 Cloudflare DNS 域名（可选）

如果您已启用 Cloudflare DNS 批量更新，则可以直接在 EdgeTunnel 的 **“自定义订阅地址”** 输入框中填入您配置的子域名（如 `你的域名.dpdns.org`）。该域名会自动解析到当前最优的多个 IP 之一，实现零配置动态切换。

> 📌 在 EdgeTunnel 的 **“Encrypted Client Hello”** 设置页面中，也可以将优选域名填入 **“PROXYIP”** 输入框，并点击 **“启用自动获取”** 来动态更新代理 IP。
