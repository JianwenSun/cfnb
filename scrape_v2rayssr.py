#!/usr/bin/env python3
"""
爬取 v2rayssr.com/cfip 页面的 CloudFlare 优选IP 表格数据
支持从URL或本地HTML文件读取
"""
...
def parse_html(html_content):
    ...
    for table in tables:
        ...
        if headers_row:
            ...
            if '排名' in headers or 'IP地址' in headers:
                ...
                if tbody:
                    ...
                    for row in rows:
                        ...
                        if cols:
                            row_data = [clean_text(col.get_text(strip=True)) for col in cols]
                            if row_data:
    ...
    if not results:
        ...
        for table in tables:
            ...
            if len(rows) > 1:
                ...
                if any(keyword in header_text for keyword in ['排名', '线路', 'IP地址', '丢包率']):
                    print(f"找到目标表格，表头: {headers}")
                    for row in rows[1:]:
                        cols = row.find_all('td')
                        if cols:
...
def save_to_file(data, filename='v2rayssr.txt'):
    if not data:
        print("没有数据可保存")
        return False
...
def main():
    ...
    if not html_content:
        ...
        print()
        print("使用方法:")
        print("  1. 直接运行: python3 scrape_v2rayssr.py")
        print("  2. 从文件读取: python3 scrape_v2rayssr.py <html文件路径>")
        print()
        print("提示: 如果网站无法访问，请手动下载HTML文件:")
        print("  - 使用浏览器访问 https://v2rayssr.com/cfip/")
        print("  - 另存为HTML文件")
        print("  - 运行: python3 scrape_v2rayssr.py <保存的文件.html>")
        return
